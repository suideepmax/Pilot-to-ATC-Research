...
## DEC-014 — AIAA SciTech 2026 competing paper obtained and read in full; positioning confirmed favorable, not a scoop

Date: 2026-09-22
Status: DECIDED (citation/positioning), closes the open item from [[DEC-012]]

Question: DEC-012 flagged a closely competing paper (AIAA SciTech 2026, on Whisper adaptation for ATC) that could only be identified via a secondary-source abstract (403 on `arc.aiaa.org` blocked direct access). The user obtained and provided the actual paper PDF. Read in full: does it preempt this project's contribution, and how should it be cited/positioned in the manuscript?

Full citation (confirmed directly from the paper, not inferred): Maaz, M., Carannante, G., Bouaynaya, N. C., Shah, S., Nichols, C., and Omezzine, A., "Efficient Domain Adaptation of Whisper for Air Traffic Control Transcription and Contextual Error Rate Evaluation," AIAA SciTech Forum, DOI: 10.2514/6.2026-1765. Affiliations: Rowan University (Glassboro, NJ), FAA William J. Hughes Technical Center (Atlantic City, NJ), ATAC Corporation (Santa Clara, CA). Funded under FAA Contract 692M15-21-T-00001. (A related Rowan University M.S. thesis by the same first author, Muhammad Maaz, advised by Bouaynaya, "When Words Fail: Why AI-Driven Context Beats Word Error Rate for Safety-Critical Air Traffic Control," approved 2025-06-30, appears to be the source work this paper condenses -- found independently before the PDF was obtained, via `rdw.rowan.edu/etd/3414/`.)

Evidence (read directly from the paper, all page/table references to the PDF as provided):

1. **Model and scope differ substantially from ours**: they fine-tune Whisper Medium (769M params, encoder-decoder ASR architecture), not a speech-LLM/SALM like Canary-Qwen-2.5B. "Transfer Learning" in their terminology means updating ALL of Whisper's parameters (our "full-decoder" analog, though for a full encoder-decoder ASR model, not a frozen-encoder+LLM-decoder composition); LoRA targets "the attention mechanisms of transformers" (unspecified exact modules) at r=128/alpha=64, 37M/5% of params.

2. **Their own result already shows full fine-tuning beats LoRA, consistently, across three data scales** (Table/Fig. 2): Benchmark data (UWB-ATCC+ATCOSIM+ATCO2 combined): Transfer Learning 7.00% vs LoRA 9.38%. FAA_80K: 8.76% vs 11.09%. FAA_500K: Transfer Learning 6.8% (Fig. 2/abstract headline) or 9.6% (Results-section prose -- the paper's own two reported numbers for this exact model disagree, not resolved anywhere in the text) vs LoRA 15.9% (Fig. 2) or 13.68% (prose). This is the SAME directional finding as this project's own headline result (full-decoder beats LoRA under matched exposure, EXP-015) -- but on a different architecture family entirely (Whisper vs. Canary-Qwen SALM). Our result should be framed as extending/confirming this finding to a different, more recent architecture class, not as a novel discovery of "full-FT beats LoRA" in general (already true per DEC-010's original novelty assessment, now doubly confirmed).

3. **Their comparison is NOT matched-protocol -- a concrete, citable methodological gap in our favor.** Their LoRA arm uses learning rate 1e-3; their Transfer Learning arm uses 1e-5 -- two orders of magnitude apart, with no documented tuning procedure or justification for the difference (contrast with this project's [[VAL-025]] per-arm LR probing). Their LoRA checkpoint is reported at epoch 20; their best-performing Transfer Learning checkpoint at epoch 6 (benchmark data) or epoch 1-4 (FAA data) -- an unequal-training-exposure comparison, structurally the same confound [[EXP-014]] identified and explicitly corrected for in this project, which their paper does not flag, discuss, or control for anywhere.

4. **Their "Contextual Error Rate" (CER) is a newly-proposed, LLM-judge-based BINARY metric** (LLaMA 3.3 70B, structured prompt, classifies each reference/hypothesis pair as "Contextually Equivalent" or "Contextually Different"; CER = count of "different" / N x 100) -- not the same as the established entity/span-level ATC metrics (CallWER, EntWER, DLR's command-recognition-rate) [[DEC-010]] already identified as the field's actual standard for critical-content evaluation. CER is itself a competing novel-metric proposal in this exact SciTech paper, not an established baseline -- if this project adopts a critical-content metric, CallWear/EntWER remain the correct choice per DEC-010's original recommendation, not CER (adopting CER would mean adopting an unvalidated, single-paper metric rather than the field's established one).

5. **Nothing in the paper resembles this project's two most distinctive findings.** No connector/bridge-initialization issue is discussed (Whisper's native encoder-decoder architecture has no analog to the SALM speech-encoder-to-LLM-decoder bridge that this project's ISS-013 finding concerns). No val_loss/WER checkpoint-selection decoupling analysis of any kind (this project's VAL-023/024/EXP-015 finding, shown three separate ways). Both remain genuinely this project's own contribution.

6. **Shared dataset, but not a shared split**: they also use UWB-ATCC (one of three ATC benchmark corpora they combine), but their own Table 1 reports train=11,290/test=2,821, differing from this project's 11,543/11,543-train_v2/2,886-test split sizes -- direct WER-number comparison against their UWB-ATCC-specific numbers (which they don't even report separately; their headline numbers are all evaluated against a held-out 10K-sample subset of their private FAA corpus, not against the UWB-ATCC test set specifically) would not be apples-to-apples and should not be attempted.

Decision: Cite this paper explicitly in the manuscript's related-work section, using the full, verified citation above (not the earlier secondary-source-only reference). Position this project's contribution as: (a) extending the full-FT-beats-LoRA finding to a different, more recent architecture class (speech-LLM/SALM vs. Whisper's native encoder-decoder), (b) doing so under a genuinely matched protocol (per-arm LR probing, epoch-normalized exposure, identical regularization) that this prior work's own comparison lacks (two-order-of-magnitude LR difference between arms, unequal/unflagged training exposure), and (c) contributing two findings with no analog in this prior work (the connector-initialization reproducibility disclosure, and the checkpoint-selection-instability finding). Do not adopt CER as this project's critical-content metric; if one is added, use the established CallWER/EntWER per DEC-010.

Consequences: DEC-012's "must be cited/positioned against, full text not yet obtained" item is now closed -- the actual paper has been read, and the positioning is more favorable to this project than the secondary-source abstract alone suggested (their own comparison has real methodological gaps this project's matched-protocol design directly addresses, which was not visible from the abstract).

Related Records: [[DEC-012]], [[DEC-010]], [[EXP-014]], [[EXP-015]], [[VAL-025]], [[ISS-013]], [[VAL-023]]
