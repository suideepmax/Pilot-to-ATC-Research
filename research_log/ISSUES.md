...
**UPDATE (2026-09-11) -- directly confirmed against v1's actual saved checkpoint, not just a simulation.** Per external-review request to stop asserting LoRA-run degeneracy without checking it: consolidated `~/canary-ft/experiments/checkpoints/step=10000-last.ckpt` and read its real optimizer state. Result: 114 LoRA-adapter optimizer entries, `exp_avg`/`exp_avg_sq` stored as `torch.float16`; **83.3% of all `exp_avg_sq` elements across the checkpoint are exactly `0.0`** (23,154,999 / 27,789,312 elements); `1 - lr*weight_decay` computed in fp16 at both the config's LRs (5e-4 top-level, 1e-6 for the checked param group) rounds to exactly `1.0`. This is checkpoint-level confirmation for v1 specifically -- not yet checked for v2/v3 individually, and this does not by itself establish that full-decoder (S3-B3) gradients would degenerate identically (LoRA adapter gradient magnitudes are not guaranteed to match full-decoder gradient magnitudes).
...
**UPDATE (2026-09-10, independent systems-architect audit) -- the confound is larger than "random bridge":** the frozen speech encoder used in every training run (`canary-1b-flash`'s own encoder) is ALSO not the released `canary-qwen-2.5b`'s encoder. Direct element-wise weight comparison between `canary-1b-flash.nemo`'s `model_weights.ckpt` and the released checkpoint's `model.safetensors`:

| tensor | maxabsdiff | flash abs-mean | canary-qwen abs-mean |
|---|---|---|---|
| `encoder.pre_encode.out.weight` | 2.182e-01 | 5.364e-02 | 6.407e-02 |
| `encoder.layers.0.self_attn.linear_q.weight` | 1.972e-01 | 1.055e-01 | 1.102e-01 |
| `encoder.layers.16.feed_forward1.linear1.weight` | 2.140e-01 | 4.172e-01 | 4.181e-01 |

Diffs of ~2e-1 on weights of magnitude ~1e-1 are far beyond bf16 rounding error (~4e-4 at that scale) -- the encoder was materially further trained during canary-qwen's own end-to-end SALM training, on top of the standalone `canary-1b-flash` checkpoint. Every run in this project freezes and uses `canary-1b-flash`'s *unadapted* encoder, not the released model's own. This means every fine-tuned condition (v1/v2/v3/S3-B3) uses an entirely different speech front-end than the released model, not merely a different bridge -- and EXP-003's zero-shot 81.49% baseline (which DOES use the released `canary-qwen-2.5b` via `SALM.from_pretrained`) is therefore a different architecture instantiation than every fine-tuned condition it has been compared against, not an apples-to-apples zero-shot reference point for this project's own fine-tuned models.

**UPDATE (2026-09-11, independent literature-search agent) -- this is a documented, maintainer-acknowledged NeMo configuration pitfall, not a novel finding, and there IS a fix.** NVIDIA maintainer `piotrzelasko` stated on the model's HuggingFace discussion page (`nvidia/canary-qwen-2.5b`, discussions/13, 2025-10-06, fetched and verified directly, not from a search snippet): "If you wish to finetune you will need to set `pretrained_weights: False` and add 1 LOC to manually load canary-qwen pretrained weights before the training starts" -- noting this capability isn't yet exposed in the YAML config system. A still-open NeMo GitHub issue (#14438, `waiting-on-customer`) shows another user independently hit the identical gap, unanswered as of this writing. Separately, `discussions/8` on the same model has the maintainer confirming the encoder point above: "For Canary-Qwen-2.5B specifically, it has to be the encoder architecture and parameters in this checkpoint, otherwise it won't work."

Every config in this project (v1/v2/v3/S3-B3) sets `pretrained_weights: True`, which is the WRONG setting for loading the released checkpoint's own trained weights -- see [[DEC-010]] for the full novelty-reassessment writeup this prompted. The fix (`pretrained_weights: False` + a manual load of `canary-qwen-2.5b`'s own state dict into the composed model before training starts) is NOT yet implemented in this project. One remaining open technical question before it can be: whether the released projection, co-trained with a DIFFERENT (further-adapted) encoder than this project's frozen `canary-1b-flash`, would even be "aligned" if loaded onto this project's encoder, or whether that combination is itself a mismatch -- not yet checked.

Related Records: [[ISS-011]], [[ISS-012]], [[VAL-010]], [[VAL-011]], [[VAL-012]], [[ISS-007]], [[DEC-010]]
...
**CORRECTION (2026-09-10, independent systems-architect audit -- caught real errors in this record, not just external critique):**

1. **The "retested and confirmed clean" claim in the paragraph above is FALSE.** The build with `.to(device=param.device)` removed (`iss014_resume1b.log`) crashed at the IDENTICAL location as the build that still had it (`iss014_resume1.log`): both traceback to `lightning/pytorch/callbacks/early_stopping.py:261, _improvement_message`. Removing the device kwarg changed nothing about this crash (see ISS-015 -- it was never the optimizer's bug). The device kwarg was never shown to be harmful in the first place; it was removed on a misdiagnosis. Kept removed anyway as an unvalidated precaution (forcing a device move on an already-correctly-sharded DTensor state tensor is a plausible latent risk), not as a "confirmed fix."

2. **Validation item 2's continuity narrative is fabricated by splicing two different runs.** `val_loss=4.1923828125` (step 15) is from `iss014_resume1.log` (the build WITH `device=`); `val_loss=4.1904...` (step 16) is from `iss014_resume1b.log` (the build WITHOUT it) -- a *different checkpoint state, different code version*. No single run ever produced this two-point trace. The two builds' fp32-preservation fix did each independently produce a finite val_loss (that part is real, and confirmed separately in each log), but the "continuity across steps" framing does not describe anything that was actually observed in one execution.

3. **The `loss_scale`/`grad_divisor` persistence fix described above is a NO-OP under the real distributed checkpoint (DCP) path used by every actual training run.** `torch.distributed.checkpoint.state_dict._get_optim_state_dict` calls this class's `state_dict()` (so the extra top-level keys ARE produced) but then only propagates the `state` and `param_groups` entries from the result -- any other top-level key, including `loss_scale`/`grad_divisor`, is silently discarded before it ever reaches a checkpoint file. Verified empirically: read DCP metadata (`torch.distributed.checkpoint.FileSystemReader(...).read_metadata()`) for three real saved checkpoints (`experiments_iss014_resume_cycle1/checkpoints/step=15-last.ckpt`, `experiments_iss014_fix_verify/checkpoints/step=15-last.ckpt`, `experiments_s3b3_gate3/checkpoints/step=843-last.ckpt`) -- zero `loss_scale` entries in any of them. Every real resume silently falls back to the YAML's static default (1024.0), exactly the bug this fix was supposed to close -- it was only ever validated in isolated single-process CPU repros, never in the real distributed path that actually runs. The compatibility warning meant to flag this is also being silently swallowed by NeMo/Lightning's logging filters (zero warning lines found in either resume log) -- there is currently no visibility into this failure at all in a real run.

The fp32 master/exp_avg/exp_avg_sq preservation (the part that actually causes NaN/Inf) IS confirmed working in the real DCP path -- verified directly via DCP metadata dtype inspection (`optimizer_0.state.*.master`/`.exp_avg`/`.exp_avg_sq` all read back as `torch.float32`). Only the loss_scale/grad_divisor persistence needs a different implementation (e.g. storing these as an entry inside an existing per-param `state[p]` dict, which IS propagated by DCP, rather than as new top-level keys, which are not) -- NOT YET IMPLEMENTED.

Status update: the NaN/Inf corruption mechanism is fixed and confirmed in the real path. The dynamic loss-scale state is NOT correctly restored on resume in the real path (silently reverts to default) -- this is lower-severity (wrong-but-plausible scale, not corruption) but still incorrect, and is open. Both this and ISS-015 must be resolved (or the resume protocol otherwise changed) before resuming any production checkpoint with the current defaults trusted to be exact.
