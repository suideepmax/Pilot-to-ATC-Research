...
- VAL-019 — S3-B3 step=843 first real deterministic WER: 39.92% (500 samples, greedy, fixed decoding bug) — worse than v1 (23.32%)/v3 (20.70%), but not a fair scope comparison yet (different optimizer, early-stopped at 8.4% of budget, mismatched encoder/bridge)
- VAL-020 — Teacher-forced audio-grounding check on S3-B3: correct-audio loss 0.63 vs permuted-audio loss 3.19 (~5x), accuracy 87%→47% — model IS acoustically grounded, not language-modeling artifact
...
- ISS-013 — HIGH: no Canary-Qwen run in this project (v1/v2/v3 or S3-B3) actually fine-tunes the released nvidia/canary-qwen-2.5b checkpoint — bridge always randomly re-initialized AND the frozen encoder itself differs materially from the released model's (weight diffs ~2e-1, far beyond rounding — canary-qwen's own SALM training further adapted the encoder). Every fine-tuned condition uses a different speech front-end than the released model, not just a different bridge; EXP-003's zero-shot baseline is a different architecture instantiation than what it's compared against [PARTIALLY RESOLVED — eval-path key-check done, encoder-mismatch confound newly documented, not yet acted on]
